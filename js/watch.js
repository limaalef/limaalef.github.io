...
// ─── Render principal ────────────────────────────
function renderWatch(match, sport) {
    renderWatchHero(match);
    renderWatchVideo(match);
    Elements.renderTvInfo(match, 'watchTvInfo');
    Elements.renderTechInfo(match, 'watchTechInfo');
    ...
    if (match.home_team?.lineup || match.away_team?.lineup) {
        renderWatchLineupsEnriched(match.home_team, match.away_team);
    }
...
function renderWatchVideo(raw) {
    const embedUrl    = raw.media?.embed_video || '';
...
// ─── Meta tags ────────────────────────────────────
function updateWatchMetaTags(match) {
    const home  = LanguageManager.t(match.home_team.name) || '';
    const away  = LanguageManager.t(match.away_team.name) || '';
    const comp  = LanguageManager.t(match.competition.name)  || '';
    const title = `${home} vs ${away} - ${comp}`;
    const desc  = `Assista ${home} ${match.score.fullTime.home ?? ''} x ${match.score.fullTime.away ?? ''} ${away} - ${comp}`;
